...
mp = struct();
GL = 5000;
mp.a = ones(1,GL); mp.a(2400:2800) = .1;
mp.e = ones(1,GL); mp.e(2600:300) = 50;
mp.r = zeros(1,GL);
mp.l = zeros(1,GL);
mp.nuc_w = 75;
mp.slide_s = 10;
x = -1:(10./(mp.nuc_w-2)):9;
mp.nuc_fp = [1./(1+exp(-1.*x))];
mp.nuc_fp = [mp.nuc_fp, 1, fliplr(mp.nuc_fp)];
%plot the footprint
clf;
hold all;
for i = 0:2:8, plot(mp.nuc_fp>(1-10^-i),'linewidth',2); end
legend(strcat('fp > 1-(10^',num2str([0:-2:-8]'),')'), 'location','south')
plot(mp.nuc_fp, 'linewidth',2);
%%

[t, s_hist] = gillespie(mp, 'n_steps', 1e4);
...
nuc_vec = ones(1,(nuc_len*2)-1); % the vector for the in_vec convolution
jump_vec = ones(1,nuc_len+jump_len); % the vector for the jumping convolutions
