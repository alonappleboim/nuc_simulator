...
for i=initial_steps:size(s_hist,1)
    ...
    if (length(nucs) == 2)
        ...
        plus_one((TSS-401) + nucs(2)) = plus_one((TSS-401) + nucs(2)) + max(s_hist(i,:));
        minus_one((TSS-401) + nucs(1)) = minus_one((TSS-401) + nucs(1)) + max(s_hist(i,:));
    ...
    if (length(nucs) == 3) % if there are three, take the two closer to the TSS:
        ...
        plus_one((TSS-401) + nucs(3)) = plus_one((TSS-401) + nucs(3)) + max(s_hist(i,:));
        minus_one((TSS-401) + nucs(2)) = minus_one((TSS-401) + nucs(2)) + max(s_hist(i,:));
    end 
end
