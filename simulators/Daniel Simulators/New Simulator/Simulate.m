function [time,full_state_history,nuc_state_history] = Simulate(model_params, varargin)
%
...
full_state_history = [extra_inputs.s0; zeros(extra_inputs.n_steps, params.genlen)]; % the state history
nuc_state_history = [extra_inputs.s0; zeros(extra_inputs.n_steps, params.genlen)];
...
for i = 1:extra_inputs.n_steps
    ...
    % not returning it multiplyed by dt so we can seperate 1, 2, 3 and 4:
	full_state_history(i+1,:) = nuc_state + 2.*REB1_state + 3.*ABF1_state + 4.*RAP1_state;
    nuc_state_history(i+1,:) = nuc_state.*dt;
    time(i+1) = time(i) + dt;
end
