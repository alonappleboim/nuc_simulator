...
%{
RSC_evict = conv(PolyAT_sites, p.RSC_evic_intensity.*gausswin(2*p.RSC_evic_length,4)','same');
REB1_evict = conv(REB1_sites, p.TF_evic_intensity.*gausswin(2*(p.REB1_width + p.nuc_width),4)','same');
ABF1_evict = conv(ABF1_sites, p.TF_evic_intensity.*gausswin(2*(p.ABF1_width + p.nuc_width),4)','same');
RAP1_evict = conv(RAP1_sites, p.TF_evic_intensity.*gausswin(2*(p.RAP1_width + p.nuc_width),4)','same');
e_rate = p.nuc_base_e_rate + RSC_evict + REB1_evict + ABF1_evict + RAP1_evict;
%}
