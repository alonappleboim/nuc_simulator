...
for gene_id=1:80
    ...
    try
        load(['C:\Users\Daniel\Documents\MATLAB\nuc_simulator\clustering\results\wt_301016\results_' num2str(gene_id) '.mat'])
    catch a
        continue
    end
    ...
    title('Simulation vs Wild Type Data - NFR Region')
    %{
    textbox_string = [...
...
good_genes_2 = [9, 19,23,24,26,29,30,31,37,40,46,58,63,67,73,74,76,77,79,86,89,90,93,94,109,136,140,144,148,149,150,155,161,163,168,169,170,175,176,177,178,180,183,184,185,189,190,195,197,200];
...
TF_evic = zeros(1,length(good_genes) + length(good_genes_2));
...
for i = 1:length(good_genes_2)
    ...
    try
        load(['C:\Users\Daniel\Documents\MATLAB\nuc_simulator\clustering\results\wt_301016\results_' num2str(good_genes_2(i)) '.mat'])
    catch a
        continue
    end
